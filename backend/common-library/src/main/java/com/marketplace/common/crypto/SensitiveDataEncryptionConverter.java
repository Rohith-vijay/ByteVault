package com.marketplace.common.crypto;
...
public class SensitiveDataEncryptionConverter implements AttributeConverter<String, String> {
    ...
    public SensitiveDataEncryptionConverter(
            @Value("${app.crypto.encryption-key:dummy_encryption_key_must_be_32_bytes_long!}") String rawKey,
            org.springframework.core.env.Environment env) {
        if (rawKey == null || rawKey.isBlank()) {
            throw new IllegalStateException("CRITICAL ERROR: Encryption key (app.crypto.encryption-key) is not configured.");
        }
        
        boolean isProduction = java.util.Arrays.asList(env.getActiveProfiles()).contains("prod");
        if (isProduction) {
            if ("dummy_encryption_key_must_be_32_bytes_long!".equals(rawKey) || rawKey.getBytes(StandardCharsets.UTF_8).length < 32) {
                throw new IllegalStateException("CRITICAL ERROR: Insecure or invalid encryption key configured in production!");
            }
        }
        ...
        try {
            ...
            log.info("[SensitiveDataEncryptionConverter] AES GCM key initialized successfully.");
        } catch (Exception e) {
            log.error("[SensitiveDataEncryptionConverter] Failed to initialize AES GCM key: {}", e.getMessage());
    ...
    public String convertToDatabaseColumn(String attribute) {
        ...
        try {
            ...
            log.error("[SensitiveDataEncryptionConverter] Encryption failed: {}", e.getMessage());
    ...
    public String convertToEntityAttribute(String dbData) {
        ...
        try {
            ...
            log.warn("[SensitiveDataEncryptionConverter] Decryption failed; returning raw value as fallback (possible legacy data): {}", e.getMessage());
            return dbData;
        }
    }
}
